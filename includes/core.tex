...
% \subsection{Dynamicka alokacia}
% \subsection{Serializacia dat}
\section{Navrh}
Jedným zo základných cieľov tejto práce je poskytnúť jednoduchšie programové prostredie pre ďalších lúštitelov alebo programátorov,
ktorý budú pri svojej práci využívať gridové prostredie.
Ako sme ukázali v predchádzajúcich kapitolách, ...
% vyvoj na lokale, spustenie na gride, struktura projektu

% \subsection{Požiadavky}

\subsection{Technológie}
% zvolene technologie: c++, mpi, boost, cmake, boost serializacia, build.sh
Dôležitou voľbou sú použité technológie.
V tejto práci sme si zvolili ako programovací jazyk \texttt{c++11},
ktorý je dostatočne rýchly a zároveň poskytuje bohatú štandartnú knižnicu. 
...
Ak by v tomto prípade odosielatel poslal dlhšiu správu ako je príjemca schopný prijať tak by tento program zlyhal.
Riešením tohto problému je dynamická alokácia pamäte pomocou štruktúry \texttt{MPI\_Status}.
...
Na lokálny vývoj sme použili linuxový operačný systém Ubuntu 16.04 LTS.
...
Ešte pred kompiláciou musíme vytvoriť súbor .project-id, ktorý by mal obsahovať
...
\begin{lstlisting}[language=sh, ]
  ...
  ./build.sh -b  
\end{lstlisting}
V tomto kroku by sme mali mat projekt úspešne skompilovaný a môžeme založiť nový projekt.
...
\begin{lstlisting}
  # vytvorenie noveho projektu s nazvom enigma
  ./build.sh -n enigma
\end{lstlisting}
Čo vytvorí nasledovnú štruktúru v priečinku projekt:
\begin{lstlisting}
  # vylistovanie projektu pomocou prikazu find aj s vystupom
  find project/enigma
  project/enigma/
  project/enigma/CMakeLists.txt
  project/enigma/enigma.pbs
  project/enigma/.gitignore
  project/enigma/src/
  project/enigma/src/main.cpp
\end{lstlisting}
...
\subsubsection{Použitie na gride hpc.stuba.sk}


% priklad pouzitia: zalozenie projektu, kompilacia, spustenie na gride

% \subsection{Genetické algoritmy}
%% teoria GA
%% Experiment s genetickymi algoritmami
%% zvolene schemy GA
