...
Riešením tohto problému je dynamická alokácia pamäte. Aby sme mohli dynamicky alokovať pamäť potrebujeme vedieť
...
ktorý budú pri svojej práci využívať grid. Takéto prostredie by malo poskytnúť spoločné stavebné bloky, ktoré
by si riešitelia medzi sebou mohli navzájom zdielať.
% vyvoj na lokale, spustenie na gride, struktura projektu, flow diagram?
...
Taktiež skopíruje \texttt{.project-id} a vynechá všetky nechcené súbory.
...
\begin{lstlisting}
  ...
  ./build.sh --sync
\end{lstlisting}
Tentokrát ak sme nespravili žiadnu zmenu na lokalnom pc, tak sa neskopiruje nič, ale stiahnu sa výstupy s programu HelloGrid.
...
%% teoria GA
%% Experiment s genetickymi algoritmami, distrubucia parametrov
%% zvolene schemy GA
