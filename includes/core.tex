...
Pri klasických šifrách sa gény zvyčajne reprezentujú pomocou abededných znakov alebo čísiel.
Ďalsím krokom je určiť prehladávaný priestor a následne formulovať účelovú funkciu. Dôležité je aj určiť veľkosť populácie,
...
naopak pri veľkých populáciach sa už nedosahuje lepšieho efektu.
...
\begin{enumerate}
\item
  Prvým krokom je vygenerovnie počiatočnej populácie $ P_0 $.
\item
  V tomto kroku sa vyhodnocuje populácia pomocou účelovej funkcie pre akuálnu populáciu $ P_k $.
\item
...
\item
  ...
  Potomkovia vytvoria skupinu $ C $, ktorá pokračuje do oprácie mutácie. 
\item
...
\end{enumerate}
Predstavená schéma je len príklad ako by GA mohol vyzerať, v skutočnosti však môžeme uvažovať o viacerých kritériach, ktoré vplývajú na GA.
...
Na obrázku \ref{img:pga} môžeme vidieť zjednodušenú blokovú shému PGA s migračným modelom. Pri takomto type prebieha výpočet jednotlivých genetických algoritmov vo viacero nezávislých, ale spolupracujúcich subpopulácia (ostrovoch). Tieto ostrovy môžu bezať na viacerých procesoroch alebo aj nezávislých počítačoch.
Ostrovy si medzi sebou po určitom počte generácií a na základe vopred určených väzieb (topológii) vymenia niektorých jedincov, napríklad najlepších v aktuálnej generácii. Potom pokračujú izolovane dalej.
Procesu výmeny jedincov medzi jednotlivými GA hovoríme migrácia.

Štrutúra toplógie medzi populáciami môže byť rôzna. Jednotlivé populácie medzi sebou nemusia byť všetky previazané.
